...
class PlaylistCreator
  ...
  class << self
    def prepare_user
      ...
      RSpotify::User.new(JSON.parse(ENV['OAUTH_TOKENS'])).tap { |user| RSpotify.client_token = user.credentials['token'] }
    end
    ...
    def create_playlist(target_name)
      ...
      CSV.foreach("output/#{target_name}.csv", headers:true, col_sep: "\t").with_index do |row, index|
        # OPTIMIZE:検索クエリ("artist:"クエリを安定化して使えないか)
        query = "#{row['title']}　#{row['artist']}"

        # 検索に失敗した場合は適当な曲で穴埋めし、後から修正
        # NOTE:現状のクエリだと正誤問わずなんかしらは返ってきてそう、バリデーションをかけるべきかも
        sleep 0.5
        track = RSpotify::Track.search(query, limit:1, market:'JP') || RSpotify::Track.search("君が代", limit:1, market:'JP')
